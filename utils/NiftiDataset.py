...
def contrast(image):
    array = np.transpose(sitk.GetArrayFromImage(image), axes=(2, 1, 0))
    if np.isnan(array).sum() != 0: print(np.isnan(array).sum())
    spacing = image.GetSpacing()
    direction = image.GetDirection()
    origin = image.GetOrigin()
...
class NifitDataSet(torch.utils.data.Dataset):
    ...
    def __init__(self, train_path,
        ...
        self.img_3t_list = lstFiles(os.path.join(train_path, '3t'))
        self.img_7t_list = lstFiles(os.path.join(train_path, '7t'))
        self.img_3thh_list = lstFiles(os.path.join(train_path, '3thh'))
        self.img_7th_list = lstFiles(os.path.join(train_path, '7th'))
        self.img_7thh_list = lstFiles(os.path.join(train_path, '7thh'))
    ...
    def __getitem__(self, index):
        ...
        img_3t_path = self.img_3t_list[index]
        ...
        if self.shuffle_labels is True:
            ...
            img_7t_path = self.img_7t_list[index]
            img_3thh_path = self.img_3thh_list[index]
            img_7th_path = self.img_7th_list[index]
            img_7thh_path = self.img_7thh_list[index]

        # read image and label
        img_3t = self.read_image(img_3t_path)        
        ...
        img_7t = self.read_image(img_7t_path)
        img_7t = Normalization(img_7t)  # set intensity 0-255
        castImageFilter.SetOutputPixelType(self.bit)
        img_7t = castImageFilter.Execute(img_7t)

        img_3thh = self.read_image(img_3thh_path)
        img_3thh = Normalization(img_3thh)  # set intensity 0-255
        castImageFilter.SetOutputPixelType(self.bit)
        img_3thh = castImageFilter.Execute(img_3thh)

        img_7th = self.read_image(img_7th_path)
        img_7th = Normalization(img_7th)  # set intensity 0-255
        castImageFilter.SetOutputPixelType(self.bit)
        img_7th = castImageFilter.Execute(img_7th)

        img_7thh = self.read_image(img_7thh_path)
        img_7thh = Normalization(img_7thh)  # set intensity 0-255
        castImageFilter.SetOutputPixelType(self.bit)
        img_7thh = castImageFilter.Execute(img_7thh)

        sample = {'3t': img_3t, '7t': img_7t, '3thh': img_3thh, '7th': img_7th, '7thh': img_7thh}
        ...
        img_3t_np = abs(sitk.GetArrayFromImage(sample['3t']))
        img_7t_np = abs(sitk.GetArrayFromImage(sample['7t']))
        img_3thh_np = abs(sitk.GetArrayFromImage(sample['3thh']))
        img_7th_np = abs(sitk.GetArrayFromImage(sample['7th']))
        img_7thh_np = abs(sitk.GetArrayFromImage(sample['7thh']))
        ...
        img_7t_np = np.transpose(img_7t_np, (2, 1, 0))
        img_3thh_np = np.transpose(img_3thh_np, (2, 1, 0))
        img_7th_np = np.transpose(img_7th_np, (2, 1, 0))
        img_7thh_np = np.transpose(img_7thh_np, (2, 1, 0))

        '''
        img_3t_np = np.pad(img_3t_np, ((10, 10), (10, 10), (0, 2)), 'edge') # 320, 320, 252
        img_7t_np = np.pad(img_7t_np, ((10, 10), (10, 10), (0, 2)), 'edge') # 320, 320, 252
        img_3thh_np = np.pad(img_3thh_np, ((10, 10), (10, 10), (0, 2)), 'edge') # 320, 320, 252
        img_7th_np = np.pad(img_7th_np, ((10, 10), (10, 10), (0, 2)), 'edge') # 320, 320, 252
        img_7thh_np = np.pad(img_7thh_np, ((10, 10), (10, 10), (0, 2)), 'edge') # 320, 320, 252
        '''

        # Plus
        img_fusion = img_7th_np + img_7thh_np
        img_fusion = 255.0 * (img_fusion - img_fusion.min()) / (img_fusion.max() - img_fusion.min())
        
        return torch.from_numpy(img_3t_np), torch.from_numpy(img_7t_np), torch.from_numpy(img_3thh_np), torch.from_numpy(img_fusion) # this is the final output to feed the network

    def __len__(self):
        return len(self.img_3t_list)

   
class PatchDataSetInterp(torch.utils.data.Dataset):
    def __init__(self, img_3t, img_7t, img_3thh, img_fusion, patch_size_z, stride_z):
        '''
        img_3t: batch_size, 288, 288, 252
        img_7t: batch_size, 288, 288, 252
        img_3thh: batch_size, 288, 288, 252
        img_fusion: batch_size, 288, 288, 252
        '''
        self.patch_size_z = patch_size_z
        self.stride_z = stride_z
        self.knum = (img_3t.shape[3] - patch_size_z) // stride_z + 1
        self.img_3t = img_3t
        self.img_7t = img_7t
        self.img_3thh = img_3thh
        self.img_fusion = img_fusion
        # each id represents a patch, for randomly select patches
        self.patch_id_list = list(range(img_3t.shape[0] * self.knum))

    def __getitem__(self, index):
        patch_id = self.patch_id_list[index]
        nii_id = patch_id // self.knum
        z_id = patch_id % self.knum
        
        img_3t_patch = self.img_3t[nii_id].unfold(2, self.patch_size_z, self.stride_z)[:,:,z_id,:] # 288, 288, 4
        img_3t_patch = img_3t_patch.unsqueeze(0) # 1, patch_size_x, patch_size_y, patch_size_z
        img_3t_patch = (img_3t_patch - 127.5) / 127.5
        
        img_7t_patch = self.img_7t[nii_id].unfold(2, self.patch_size_z, self.stride_z)[:,:,z_id,:] # 288, 288, 4
        img_7t_patch = img_7t_patch.unsqueeze(0) # 1, patch_size_x, patch_size_y, patch_size_z
        img_7t_patch = (img_7t_patch - 127.5) / 127.5

        img_3thh_patch = self.img_3thh[nii_id].unfold(2, self.patch_size_z, self.stride_z)[:,:,z_id,:] # 288, 288, 4
        img_3thh_patch = img_3thh_patch.unsqueeze(0) # 1, patch_size_x, patch_size_y, patch_size_z
        img_3thh_patch = (img_3thh_patch - 127.5) / 127.5

        img_fusion_patch = self.img_fusion[nii_id].unfold(2, self.patch_size_z, self.stride_z)[:,:,z_id,:] # 288, 288, 4
        img_fusion_patch = img_fusion_patch.unsqueeze(0) # 1, patch_size_x, patch_size_y, patch_size_z
        img_fusion_patch = (img_fusion_patch - 127.5) / 127.5

        return img_3t_patch, img_7t_patch, img_3thh_patch, img_fusion_patch
...
class Augmentation(object):
    ...
    def __call__(self, sample):
        ...
        if choice == 0:  # no augmentation
            ...
            img_3t, img_7t, img_3thh, img_7th, img_7thh = sample['3t'], sample['7t'], sample['3thh'], sample['7th'], sample['7thh']
            return {'3t': img_3t, '7t': img_7t, '3thh': img_3thh, '7th': img_7th, '7thh': img_7thh}
        ...
        if choice == 1:  # Additive Gaussian noise
            ...
            img_3t, img_7t, img_3thh, img_7th, img_7thh = sample['3t'], sample['7t'], sample['3thh'], sample['7th'], sample['7thh']
            img_3t = self.noiseFilter.Execute(img_3t)
            img_7t = self.noiseFilter.Execute(img_7t)
            img_3thh = self.noiseFilter.Execute(img_3thh)
            img_7th = self.noiseFilter.Execute(img_7th)
            img_7thh = self.noiseFilter.Execute(img_7thh)
        ...
        if choice == 3:  # Random rotation
            ...
            img_3t = rotation3d_image(img_3t, theta_x, theta_y, theta_z)
            img_7t = rotation3d_image(img_7t, theta_x, theta_y, theta_z)
            img_3thh = rotation3d_image(img_3thh, theta_x, theta_y, theta_z)
            img_7th = rotation3d_image(img_7th, theta_x, theta_y, theta_z)
            img_7thh = rotation3d_image(img_7thh, theta_x, theta_y, theta_z)
        ...
        if choice == 4:  # BSpline Deformation
            ...
            img_3t, img_7t, img_3thh, img_7th, img_7thh = sample['3t'], sample['7t'], sample['3thh'], sample['7th'], sample['7thh']
            spline_order = 3
            domain_physical_dimensions = [img_3t.GetSize()[0] * img_3t.GetSpacing()[0],
                                          img_3t.GetSize()[1] * img_3t.GetSpacing()[1],
                                          img_3t.GetSize()[2] * img_3t.GetSpacing()[2]]

            bspline = sitk.BSplineTransform(3, spline_order)
            bspline.SetTransformDomainOrigin(img_3t.GetOrigin())
            bspline.SetTransformDomainDirection(img_3t.GetDirection())
            ...
            img_3t = sitk.Resample(img_3t, bspline)
            img_7t = sitk.Resample(img_7t, bspline)
            img_3thh = sitk.Resample(img_3thh, bspline)
            img_7th = sitk.Resample(img_7th, bspline)
            img_7thh = sitk.Resample(img_7thh, bspline)
        ...
        if choice == 5:  # Random flip
            ...
            img_3t = flipit(img_3t, axes)
            img_7t = flipit(img_7t, axes)
            img_3thh = flipit(img_3thh, axes)
            img_7th = flipit(img_7th, axes)
            img_7thh = flipit(img_7thh, axes)
        ...
        if choice == 6:  # Brightness
            ...
            img_3t = brightness(img_3t)
            img_7t = brightness(img_7t)
            img_3thh = brightness(img_3thh)
            img_7th = brightness(img_7th)
            img_7thh = brightness(img_7thh)
        ...
        if choice == 7:  # Contrast
            ...
            img_3t = contrast(img_3t)
            img_7t = contrast(img_7t)
            img_3thh = contrast(img_3thh)
            img_7th = contrast(img_7th)
            img_7thh = contrast(img_7thh)
